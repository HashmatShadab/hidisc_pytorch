...
def get_mean_std(normalize):
    if normalize is None:
        log.info("Using default normalization from dataset class")
        return None, None
    elif normalize == 'imagenet':
        mean, std = transforms.IMAGENET_MEAN, transforms.IMAGENET_STD
        log.info('Using imagenet normalization')
        return mean, std
    elif normalize == 'clip':
        mean, std = transforms.CLIP_MEAN, transforms.CLIP_STD
        log.info('Using clip normalization')
        return mean, std
    else:
        raise NotImplementedError

def plot_grid(w, norm=False):
...
def init_distributed_mode(args):
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        log.info('Using distributed mode: 1')
    elif 'SLURM_PROCID' in os.environ:
        print("Using SLURM with {} nodes".format(os.environ['SLURM_NNODES']))
        ...
        log.info('Using distributed mode: slurm')
        log.info(f"world: {os.environ['WORLD_SIZE']}, rank:{os.environ['RANK']},"
...
class MetricLogger(object):
    ...
    def log_every(self, iterable, print_freq, header=None):
        ...
        log.info(f"{header} Total time: {total_time_str} ({total_time / len(iterable):.4f} s / it)")
